...
import { Exchange, type ExchangeRepo } from '../domain'
...
describe('ProcessExchangeUseCase', () => {
  ...
  it('should throw error if limit exceeded (up)', async () => {
    ...
    await expect(useCase.exec(highRateExchange)).rejects.toThrow('Exchange limit exceeded')
  ...
  it('should throw error if limit exceeded (down)', async () => {
    ...
    await expect(useCase.exec(lowRateExchange)).rejects.toThrow('Exchange limit exceeded')
