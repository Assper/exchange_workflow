...
import type { UseCase } from 'src/shared'
import {
  type Exchange,
  ExchangeLimit,
  type ExchangeRepo,
...
import { EXCHANGE_REPO, EXCHANGE_WORKFLOW, LIMIT_REPO } from '../provider-tokens'
...
export class ProcessExchangeUseCase implements UseCase<Exchange, void> {
  ...
  private async getConfigByExchange(exchange: Exchange): Promise<Limit | null> {
    ...
    return (
      configs.find((config) => config.from === exchange.from && config.to === exchange.to) || null
    )
  }
